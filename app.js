...
app.get('/', (req, res) => {
  ...
  Item.find({}, function (err, foundItems) {
    // be careful the error parameter first then the returned list
    if (err) {
      console.log(err);
      ...
      // why we didn't just render the data without else?? as we need the query to be excuted two times (one for check for the data exists or not and insert it and the second for getting the data inserted to be rendered)
...
app.get('/:customListName', function (req, res) {
  ...
  List.findOne({ name: customListName }, function (err, foundList) {
    //we used findOne() as it returns null if item not found and returns an object if found but find() returns cursor if item not found and returns list of objects if found.
    if (!err) {
      if (!foundList) {
        // newCreated = true;
        const list = new List({
    ...
    setTimeout(() => {
      List.findOneAndUpdate(
        ...
        function (err, foundlist) {
          //'findOne' finds the record we want to update and 'AndUpdate' updates the items array in the document
          if (!err) {
            res.redirect('/' + listName);
          }
        }
